...
const incrementLebuff = (_decrement = false) => {
  if (!timers[0].hasStarted) {
    return;
  }

  const counter = document.getElementById("lebuff-icon-text");
  let number = parseInt(counter.innerText, 10);
  if (_decrement) {
    number--;
    runData.pushups--;
...
const initChallengeRunBody = () => {
  ...
  if (!settings.lebuffMode) {
    addClass(document.getElementById("lebuff-icon-container"), "hidden");
  }
}
...
const start = () => {
  if (!initialized) {
    ...
    document.getElementById("lebuff-icon-text").innerText = settings.lebuffPushupCount;
...
const initVars = () => {
  ...
  runData = {
    ...
    solvedLevelIds: new Set(),
    chosenLevelCache: new Set(),
    pushups: settings.lebuffPushupCount,
...
const processScoreScreen = () => {
  ...
  if (settings.lebuffMode) {
    const lebuffPushupEl = document.getElementById("score-screen-pushups");
    lebuffPushupEl.innerText = runData.pushups;
  } else {
    const lebuffPushupContainer = document.getElementById("score-screen-lebuff-container");
    addClass(lebuffPushupContainer, "hidden");
  }
